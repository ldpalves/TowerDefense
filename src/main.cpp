...
void DrawVirtualObject(VirtualObject& virtual_object); // Desenha um objeto armazenado em g_VirtualScene
...
std::vector<VirtualObject *> virtual_objects;
...
VirtualObject allview(4,"allview", Matrix_Identity());
VirtualObject allproject(5,"allproject", Matrix_Identity());
...
/*
    PrintVector(lRayStart_camera);
    PrintVector(lRayStart_world);
    PrintVector(lRayEnd_camera);
    PrintVector(lRayEnd_world);
*/
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(width, height, "INF01047 - Seu Cartao - Seu Nome", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Abaixo definimos as varáveis que efetivamente definem a câmera virtual.
        // Veja slides 195-227 e 229-234 do documento Aula_08_Sistemas_de_Coordenadas.pdf.
        glm::vec4 camera_view_vector     = glm::vec4(0.5f, -1.0f, 0.0f, 0.0f);
        ...
        allview.setModel(view);
        allproject.setModel(projection);
        ...
        if(isWPressed || isSPressed || isAPressed || isDPressed) {
            colision = bunny.CheckCollision(sphere);
        }
        float movement = speed*time_delta;
        if(isWPressed) bunny.move(movement,0.0f,0.0f, virtual_objects);
        if(isSPressed) bunny.move(-movement,0.0f,0.0f, virtual_objects);
...
void DrawVirtualObject(VirtualObject& virtual_object)
{
    glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(virtual_object.getModel()));
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    //-------------------------
    //Movimentação Camera Livre

    // Se o usuário apertar a tecla W, a key pressionada vira true, ao soltar, vira false.
    if (key == GLFW_KEY_W && action == GLFW_PRESS) isWPressed = true;
    else if (key == GLFW_KEY_W && action == GLFW_RELEASE) isWPressed = false;

    // Se o usuário apertar a tecla S, a key pressionada vira true, ao soltar, vira false.
    if (key == GLFW_KEY_S && action == GLFW_PRESS) isSPressed = true;
    else if (key == GLFW_KEY_S && action == GLFW_RELEASE) isSPressed = false;

    // Se o usuário apertar a tecla A, a key pressionada vira true, ao soltar, vira false.
    if (key == GLFW_KEY_A && action == GLFW_PRESS) isAPressed = true;
    else if (key == GLFW_KEY_A && action == GLFW_RELEASE) isAPressed = false;
